...
export const PrintableSheet: React.FC<PrintableSheetProps> = ({ kit, onClose }) => {
  return (
    <div className="fixed inset-0 z-50 bg-white text-slate-900 overflow-y-auto p-8 print:p-0">
      {/* Top action bar (hidden in print) */}
      <div className="max-w-4xl mx-auto flex items-center justify-between pb-6 mb-8 border-b border-slate-200 print:hidden">
        <button
          onClick={onClose}
        ...
        <button
          ...
          className="flex items-center space-x-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-sm font-semibold shadow-sm"
        >
      ...
      <article className="max-w-4xl mx-auto space-y-8 font-serif leading-relaxed">
        {/* Header */}
        <header className="border-b-2 border-slate-900 pb-4">
          <div className="text-xs uppercase tracking-widest text-slate-500 font-sans">
            AI Interview Preparation Dossier
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-950 mt-1">
            {kit.role.title} • {kit.source.company}
          </h1>
          <div className="text-xs text-slate-600 font-sans mt-2 flex items-center space-x-4">
            <span>Seniority: {kit.role.seniority}</span>
            <span>Target Timeline: {kit.schedule.days_available} Days</span>
        ...
        <section className="space-y-2">
          <h2 className="text-sm font-sans font-bold uppercase tracking-wider text-indigo-900 border-b pb-1">
            1. Company Intelligence Brief
          </h2>
          <p className="text-sm text-slate-800">{kit.company_brief.summary}</p>
          <p className="text-sm text-slate-800 font-medium">{kit.company_brief.what_they_do}</p>
        </section>
          ...
          <div className="space-y-4">
            {kit.questions.map((q) => (
              <div key={q.id} className="p-3 border border-slate-200 rounded text-xs space-y-1.5 break-inside-avoid">
                <div className="flex items-center justify-between font-sans text-[11px] text-slate-500">
